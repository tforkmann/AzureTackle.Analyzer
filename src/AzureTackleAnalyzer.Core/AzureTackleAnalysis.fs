...
module SqlAnalysis =
    ...
    let findQuery (operation: SqlOperation) =
        operation.blocks
        |> List.tryFind (function | AzureTableAnalyzerBlock.Query(query, range) -> true | _ -> false)
        |> Option.map(function | AzureTableAnalyzerBlock.Query(query, range) -> (query, range) | _ -> failwith "should not happen")

    let findTransactions (operation: SqlOperation) =
        operation.blocks
        |> List.tryFind (function | AzureTableAnalyzerBlock.Transaction queries -> true | _ -> false)
        |> Option.map(function
            | AzureTableAnalyzerBlock.Transaction queries -> queries
    ...
    let findParameters (operation: SqlOperation) =
        operation.blocks
        |> List.tryFind (function | AzureTableAnalyzerBlock.Parameters(parameters, range) -> true | _ -> false)
        |> Option.map(function | AzureTableAnalyzerBlock.Parameters(parameters, range) -> (parameters, range) | _ -> failwith "should not happen")

    let findColumnReadAttempts (operation: SqlOperation) =
        operation.blocks
        |> List.tryFind (function | AzureTableAnalyzerBlock.ReadingColumns(attempts) -> true | _ -> false)
        |> Option.map(function | AzureTableAnalyzerBlock.ReadingColumns(attempts) -> attempts | _ -> failwith "should not happen")
    ...
    let analyzeOperation (operation: SqlOperation) (connectionString: string) (schema: InformationSchema.DbSchemaLookups) =
        let skipAnalysis =
            operation.blocks
            |> List.exists (fun block -> block = AzureTableAnalyzerBlock.SkipAnalysis)
