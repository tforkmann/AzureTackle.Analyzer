...
module AzureAnalysis =
    ...
    let extractParametersAndOutputColumns(connectionString, tableName, dbSchemaLookups) =
        task {
            try
    ...
    let findQuery (operation: AzureOperation) =
        operation.blocks
        |> List.tryFind (function | AzureAnalyzerBlock.TableQuery(query, range) -> true | _ -> false)
        |> Option.map(function | AzureAnalyzerBlock.TableQuery(query, range) -> (query, range) | _ -> failwith "should not happen")


    let findParameters (operation: AzureOperation) =
        operation.blocks
    ...
    let analyzeParameters parameters parametersRange (requiredParameters: InformationSchema.Filters list) =
        ...
        | None ->
            if not (List.isEmpty requiredParameters) then
                let missingParameters =
                    ...
                    |> String.concat ", "
                    |> sprintf "Missing parameters [%s]. Please use AzureTable.parameters to provide them."
                        ...
                        if isRedundant providedParam then
                            ...
                            | Some requiredParam ->
                                ...
                                if providedParam.paramFunc = "AzureTable.parameter" then
                                    ...
                                    | ("text"|"json"|"xml") ->
                                        if requiredParam.IsNullable then
                                            if providedParam.paramFunc <> "AzureTable.text" && providedParam.paramFunc <> "AzureTable.textOrNone" && providedParam.paramFunc <> "AzureTable.string" && providedParam.paramFunc <> "AzureTable.textOrNone" && providedParam.paramFunc <> "AzureTable.dbnull"
                                            then yield typeMismatch [ "AzureTable.text"; "AzureTable.string"; "AzureTable.textOrNone"; "AzureTable.stringOrNone"; "AzureTable.dbnull" ]
                                        else
                                            if providedParam.paramFunc <> "AzureTable.text" && providedParam.paramFunc <> "AzureTable.string"
                                            then yield typeMismatch [ "AzureTable.text"; "AzureTable.string" ]
                                    | _ ->
                                        ()
    ...
    let formatColumns (availableColumns: InformationSchema.Column list) =
        availableColumns
        |> List.map (fun column ->
            sprintf "| -- %s of type %A" column.Name column.EntityProperty
        )
        |> String.concat "\n"
    ...
    //                     | ("text"|"json"|"xml"|"jsonb") when attempt.funcName <> "AzureTable.readString" ->
    ...
    //                     | ("timestamp"|"timestamp without time zone") when attempt.funcName <> "AzureTable.readTimestamp" ->
    //                         yield typeMismatch [ "AzureTable.readTimestamp" ]
    //                     | ("timestamptz"|"timestamp with time zone") when attempt.funcName <> "AzureTable.readTimestampTz" ->
    //                         yield typeMismatch [ "AzureTable.readTimestampTz" ]
    //                     | ("interval" | "time without time zone" | "time") when attempt.funcName <> "AzureTable.readTime" ->
    //                         yield typeMismatch [ "AzureTable.readTime" ]
    //                     | _ ->
    //                         ()
    //                 else
    ...
    let databaseSchema connectionString tableName =
        task {
            try
                let! entities = InformationSchema.getAzureTableEntity(connectionString,tableName)
                return Ok (entities)
    ...
    let analyzeOperation (operation: AzureOperation) (connectionString: string) tableName (schema: InformationSchema.TableEntity) =
        task {
            ...
            | Some (query, queryRange) ->
                ...
                | Result.Error queryError ->
                    return [ createWarning queryError queryRange ]
